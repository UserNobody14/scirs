...
use crate::layers::{Dense, Dropout, Layer, LayerNorm, MultiHeadAttention};
// Note: PatchEmbedding not yet implemented
...
#[derive(Clone, Debug)]
struct TransformerMlp<F: Float + Debug + ScalarOperand + Send + Sync + NumAssign> {
    dense1: Dense<F>,
    dense2: Dense<F>,
}
...
impl<
        F: Float + Debug + ScalarOperand + Clone + Send + Sync + SimdUnifiedOps + NumAssign + 'static,
    > Clone for TransformerEncoderBlock<F>
{
    fn clone(&self) -> Self {
        Self {
...
// pub struct VisionTransformer<F: Float + Debug + ScalarOperand + Clone + Send + Sync + SimdUnifiedOps + NumAssign>
// {
